...
object RFtree {
  ...
  //    var parent: Int,
  //    /**var err: Double,*/
  //    var c: Map[String, Double], var miss: Map[String, Double], var size: Int, var leaf: Int) {
  //
  //  }
  class Node(var index: Int, var split: String, var i: Int,
    var left: Int, var right: Int,
    var parent: Int,
    var splitprob: Array[Double], var prob: Map[String, Double], var size: Int, var leaf: Int)
  ...
  def buildDT(nodes: HashMap[Int, Node], data: ArrayBuffer[LabeledFeature],
    ...
    val isLeaf = classFraction.filter(p => p._2 > 0.9999).size > 0
    ...
    if (!isLeaf) {
      ...
      //    nodes(index) = new Node(index, bestSplit, bestAttr, 2 * index, 2 * index + 1, -1, parentDist, bestDist(2).toMap, data.size, 2)
      ...
      if (left.size > N && right.size > N) {
        ...
        val parent = if (index % 2 == 0) { index / 2 } else { (index - 1) / 2 }
        val l = labelfor(data)
        val node = new Node(index, "", -1, -1, -1, parent, null, l, data.size, 1)
        nodes(index) = node
        // println(node.c)
        node
      }
  ...
  def labelfor(data: ArrayBuffer[LabeledFeature]): Map[String, Double] = {

    var iClass = data.groupBy(f => f.label).map(f => (f._1, f._2.map(f => f.weight).sum))
    val classSum = iClass.values.sum
    val classFraction = iClass.map(f => (f._1, f._2 * 1.0 / classSum))
    //      var max = -1.0
    //      var label = ""
  ...
  def minIndex(Arr: Array[Double]): Int = {
    ...
    var min = Arr(0)
    var j = 0
    Arr.map(f => { if (f < min && f > 0.0) { min = f; i = j }; j += 1 })
    i
  }
  def splitNonimal(instances: ArrayBuffer[LabeledFeature],
    ...
    for (currCutPoint <- allValue.toIterator) {
      ...
      var dist = Array.fill(3)(HashMap[String, Double]())
      ...
      //      miss.map(f => {
      //        val label = f.label
      //        dist(1)(label) = dist(1).getOrElse(label, 0.0) + f.weight
      //      })
  ...
  def instanceFor(nodes: HashMap[Int, Node], lev: Int, x: Array[String], numIdx: HashSet[Int]): Map[String, Double] = {
    ...
    //    
    val d = if (i == -1) true else x(i).equalsIgnoreCase("?")
    //
    val m = if (d) {
      if (i > 0) {
        val p = n.splitprob
        val t1 = instanceFor(nodes, lev * 2, x, numIdx)
        val t2 = instanceFor(nodes, lev * 2, x, numIdx)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val nodes = classifier(insts)
    printTree(nodes, nodes(1), 0)
    insts.data.map(f => {
      println(instanceFor(nodes, 1, f.features, numIdx) + "=>" + f.label)
    })
  }
