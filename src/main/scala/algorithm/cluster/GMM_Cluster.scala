...
object GMM_Cluster {
  ...
  def cluster(data:Array[Array[Double]],
    ...
    while(!isbreak){     
      ...
      val N = data.length
      val pxt = transpose(px).map(f=>f.sum)
      ...
      for(k<-0 until pxt.length){
        ...
        for(i<- 0 until N){
          ...
          sum1 = plus(multi(dot(transpose(Array(xi_u)),Array(xi_u)),px(i)(k)),sum1)
        }
  ...
  def prob(data:Array[Array[Double]],
    ...
    for(i<- 0 until data.length){
      for(k<-0 until pPi.length){
    	  ...
    	  val temp = math.exp(dot(dot((Array(x_u)),inv),transpose(Array(x_u)))(0)(0)*(-0.5))
