...
object BayesNet {
  ...
  def index(file: String) {
    ...
    buff.getLines.toArray.map(line => {
      ...
      //      if (!idx.contains(0)) { idx.put(0, new HashSet[String]()) }
      //      if (!idx(0).contains(label)) { idx(0) += label }
      var features = arr.reverse //.slice(0, arr.length - 1)
      //features = Array(label,features:_*)
  ...
  def distributionForInstance(instance: LabeledFeature): Array[Double] = {
  ...
  def main(args: Array[String]): Unit = {
    //println(logGamma(0.12345)*logGamma(0.13335))
    //println(logGamma(0.12345+0.13335))
    //println(lnGamma(0.12345))
    index("E:/books/spark/ml/decisionTree/test.txt")
    data.map(f => { distributionForInstance(f) })
  }
